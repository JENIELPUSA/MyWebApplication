...
const LaboratoryTable = ({
    ...
    currentPage,
    totalLaboratory,
    setLaboratoryPerPage // This was missing in your code
}) => {
    const [searchTerm, setSearchTerm] = useState('');



    const totalPages = Math.ceil(totalLaboratory / laboratoryPerPage); // Calculate total pages
    ...
    if (loading)
        ...
        const filteredLaboratory = laboratory.filter((laboratorys) =>
            laboratorys.LaboratoryName.toLowerCase().includes(searchTerm.toLowerCase())
        );
    

    const isNoLaboratory = filteredLaboratory.length === 0; // Fixed here
        ...
        <div className="flex flex-col p-6">
            <div>
                <div className="flex justify-between items-center mb-4">
                    ...
                    />
                    <select
                        value={laboratoryPerPage}
                        onChange={(e) => setLaboratoryPerPage(Number(e.target.value))}
                ...
                <div className="flex-col text-gray-700 bg-white shadow-md rounded-xl bg-clip-border px-4 py-4">
                    <div className="overflow-y-auto" style={{ maxHeight: '400px' }}>
                        <table className="w-full text-left table-auto border-collapse">
                            <thead>
                                <tr>
                                    <th className="p-4 border-b border-gray-300 bg-blue-50 text-gray-700">Laboratory Name</th>
                                    <th className="p-4 border-b border-gray-300 bg-blue-50 text-gray-700">Department Name</th>
                                    <th className="p-4 border-b border-gray-300 bg-blue-50 text-gray-700">Encharge Name</th>
                                    <th className="p-4 border-b border-gray-300 bg-blue-50 text-gray-700">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                {isNoLaboratory ? (
                                    ...
                                    filteredLaboratory.map((laboratory) => (
                                        <tr key={laboratory._id} className="hover:bg-gray-100 transition-colors">
                                            ...
                                            <td className="border border-gray-300 p-2 flex space-x-2">
                                                ...
                                                <button
                                                    onClick={() => onDeleteLab(laboratory._id)}
                    ...
                    <div className="flex space-x-1 justify-center mt-4">
                        <button
                            className={`rounded-full border border-slate-300 py-2 px-3 text-center text-sm transition-all shadow-sm text-slate-600 hover:text-white hover:bg-gradient-to-r from-blue-500 to-indigo-500 ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
                        ...
                        <button
                            className={`rounded-full border border-slate-300 py-2 px-3 text-center text-sm transition-all shadow-sm text-slate-600 hover:text-white hover:bg-gradient-to-r from-blue-500 to-indigo-500 ${currentPage === totalPages || isNoLaboratory ? 'opacity-50 cursor-not-allowed' : ''}`}
                            onClick={() => paginate(currentPage + 1)}
                            disabled={currentPage === totalPages || isNoLaboratory}
                        >
                            Next
                        </button>
                    </div>
