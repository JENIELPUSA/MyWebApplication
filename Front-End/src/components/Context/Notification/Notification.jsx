...
const Notification = ({ toggleTechnicianModal }) => {
  ...
  const { setSendPatch, setSendMsg, setSendPost } = useContext(MessagePOSTcontext);
  const { request, setRequest, fetchRequestData } = useContext(RequestDisplayContext);
  const { ToAdminCount, ToAdmin, msg, fetchDisplayMessgae, msgcount } = useContext(MessageDisplayContext);
  ...
  useEffect(() => {
    if (role === "Admin") {
      ...
      const filteredList = (request || []).filter((item) => {
        ...
        const targetName = fullName ? fullName.toString().trim().toLowerCase() : "";
  ...
  useEffect(() => {
    const handleAddMaintenance = (data) => {
      setRequest((prev) => {
        const exists = prev.some((item) => item._id?.toString() === data._id?.toString());
    ...
    const handleUpdateMaintenance = (data) => {
      setRequest((prev) => {
        const index = prev.findIndex((item) => item._id?.toString() === data._id?.toString());
        if (index !== -1) {
          const updated = [...prev];
  ...
  const updateRequest = async ({ url, updateData, socketEvent, msgId }) => {
    try {
      ...
      if (response.status === 200 || response.data.status === "success") {
        if (socketEvent) {
          socket.emit(socketEvent, { message: "Update made!", data: response.data.data });
  ...
  return (
    <div className="relative text-blue-700 cursor-pointer">
      ...
      {/* Badge Logic */}
      {((role === "Admin" && (CountBadge > 0 || ToAdminCount?.length > 0)) ||
        ...
        {isNotificationOpen && (
          ...
          >
            <div className="flex justify-between items-center mb-3 border-b border-blue-700 pb-2">
              <span className="font-black uppercase text-xs tracking-widest text-yellow-400">Notifications</span>
              <span className="text-[10px] bg-blue-800 px-2 py-0.5 rounded text-blue-200">System</span>
            </div>
            ...
            {role === "Admin" ? (
              (request?.length > 0 || ToAdmin?.length > 0) ? (
                [...request, ...ToAdmin]
                  ...
                  .slice(0, 30)
                  .map((req, index) => (
                    <div key={index} className="mb-4 bg-blue-800/50 p-3 rounded-lg border border-blue-700/50 hover:border-yellow-400/50 transition-all">
                      <h1 className="text-xs font-black text-yellow-400 uppercase">
                        {req.Department !== "N/A" && req.laboratoryName !== "N/A"
                          ? `${req.Department} / ${req.laboratoryName}`
                          : req.Department !== "N/A" ? req.Department : req.laboratoryName}
                      </h1>
                      <h4 className="text-[11px] font-bold text-white mt-1">
                        {req.CategoryName && req.EquipmentName ? `${req.CategoryName} / ${req.EquipmentName}` : req.CategoryName || req.EquipmentName}
                      </h4>
                      <time className="text-[9px] text-blue-300 block mt-1 italic">{new Date(req.DateTime).toLocaleString()}</time>
                      <p className="text-[11px] text-gray-200 mt-2 leading-relaxed">
                        {ToAdmin.includes(req) ? req.message : req.read === false && req.Ref ? (
                          <>Maintenance Request: <span onClick={() => toggleTechnicianModal(req)} className="text-yellow-400 underline font-bold cursor-pointer">{req.Ref}</span></>
                        ) : req.message}
                      </p>
                    </div>
                  ))
              ) : (
                <div className="py-10 text-center text-blue-300 text-[10px] italic">No New Admin Requests</div>
              )
            ) 
            ...
            : (role === "User" || role === "Technician") ? (
              (msg && msg.length > 0) ? (
                [...msg]
                  ...
                  .sort((a, b) => new Date(b.DateTime) - new Date(a.DateTime))
                  .map((req, index) => (
                    <div key={index} className="mb-4 bg-blue-800/50 p-3 rounded-lg border border-blue-700">
                      <time className="text-[9px] text-blue-300 italic mb-1 block">{new Date(req.DateTime).toLocaleString()}</time>
                      <p className="text-[11px] text-white font-semibold">
                        {role === "Technician" && req.types === "AssignedTechnician"
                          ? `Admin assigned you to troubleshooting in ${req.laboratoryName} / Ref#: ${req.Ref}`
                          : req.message}
                      </p>

                      {/* Technician Accept Button */}
                      {role === "Technician" && req.Status === "Pending" && req.types === "AssignedTechnician" && (
                        <button
                          onClick={() => onSelectMessage(req.RequestID, req._id)}
                          disabled={clickedRows.has(req.RequestID) || loading === req.RequestID}
                        ...
                        >
                          {loading === req.RequestID ? <LoadingSpinner /> : "Accept Task"}
                        </button>
                      )}

                      {/* User Actions (Feedback/Approve) */}
                      {role === "User" && req.read === false && (
                        <div className="mt-2">
                          {req.message.toLowerCase().includes("verification") ? (
                            <button
                            ...
                            >
                              {loading === req ? <LoadingSpinner /> : "Approve Remark"}
                            </button>
                          ) : req.message.toLowerCase().includes("feedback") ? (
                            <div className="space-y-2">
                              <textarea
                                ...
                                onChange={(e) => setValues({ ...values, [req.RequestID]: e.target.value })}
                              />
                              <button
                                onClick={() => acceptVerification(req, req._id)}
                                disabled={!values[req.RequestID] || clickedRows.has(req)}
                                className="w-full bg-yellow-400 hover:bg-yellow-500 text-blue-900 text-[10px] font-black uppercase py-2 rounded-lg transition-all"
                              >
                                {loading === req ? <LoadingSpinner /> : "Send Feedback"}
                              </button>
                            </div>
                          ) : null}
                        </div>
                ...
                <div className="py-10 text-center">
                  <p className="text-blue-300 text-xs font-bold uppercase tracking-widest italic">No New Notifications</p>
                </div>
              )
            ) : null}
          </motion.div>
